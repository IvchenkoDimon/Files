...
void add_file_extention_dhcpd(char szStr[]);
...
//1)Создаем поток вывода в файл:
	...
	add_file_extention_dhcpd(dst_filename);
	...
	if (fin.is_open())
	{
		while (!fin.eof())
		{
			...
			if (is_mac(szMACbuffer)) cout << szMACbuffer << "\t";
			if (is_ip(szIPbuffer)) cout << szIPbuffer << "\n";
		}
	}
	fout.close();
	fin.close();
	...
	cin >> szStr;
	cout << szStr << endl;
	cout << is_mac(szStr) << endl;
	cout << strlen(szStr) << endl;
	cout << is_ip(szStr) << endl;
#endif // MAC_IP_CHECK
}
...
void add_file_extention(char szStr[])
{
	...
	strcat(szStr, ".txt"); //Конкатенация строк = слияние строк.
}
void add_file_extention_dhcpd(char szStr[])
{
	int size = strlen(szStr);
	if (strcmp(szStr + size - 6, ".dhcpd") == 0)return;
	strcat(szStr, ".dhcpd");
}